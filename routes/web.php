...
Route::post('/statistics/tracks', function (Request $request) {
    ...
    if ($startDate && $endDate) {
        ...
        $stats_results = DB::table('playbackstats_tracks')->where('starttime', '>=', $subDaysTime)->where(['accountid' => $account_id])->orderBy('listeners', 'DESC')->orderBy('duration', 'DESC')->chunk(100, function ($stats) use (&$playbackStats, &$total_duration) {
            // $playbackStats = [];
